...
async function ExportAllDiagrams() {
  ...
  try {
    const entries = [];
    for (const dKey of exportedKeys) {
      let hasMd = false;
      try {
        await workDirectoryHandle.getFileHandle(`${dKey}.md`, { create: false });
        hasMd = true;
      } catch (_) {
        // .md not present — that is fine, it is optional
      }
      entries.push({ name: dKey, hasMd });
    }
    entries.sort((a, b) => a.name.localeCompare(b.name));

    const readmeText = buildReadmeMarkdown(entries);
    const readmeHandle = await workDirectoryHandle.getFileHandle('README.md', { create: true });
    const readmeWritable = await readmeHandle.createWritable();
    await readmeWritable.write(readmeText);
    await readmeWritable.close();
    readmeWritten = true;
  } catch (err) {
    console.error('README.md write failed:', err);
    failed.push(`README.md: ${err?.message || err}`);
  }

  let msg = `Exported ${exported.length} diagram(s) to JSON in the selected work directory.`;
  if (readmeWritten) msg += `\nREADME.md updated.`;
  if (failed.length > 0) {
...
function buildReadmeMarkdown(entries) {
  const lines = [];
  lines.push('# Moose2Model Work Folder');
  lines.push('');
  lines.push('**This file is auto-generated by Moose2Model2 (https://www.moose2model.org)** on every');
  lines.push('"Export All Diagrams" run. Do not edit it manually — your changes will be overwritten on');
  lines.push('the next export. If you want to keep manually-maintained notes about this folder, create');
  lines.push('a separate file `README2.md`. That file is never written or overwritten by the tool.');
  lines.push('');
  lines.push('This folder contains diagrams of a software architecture.');
  lines.push('');
  lines.push('## What is a SOMIX model?');
  lines.push('');
  lines.push('SOMIX is a small meta-model for software analysis. A SOMIX model represents a codebase as');
  lines.push('a graph of typed elements and relationships.');
  lines.push('');
  lines.push('Element types:');
  lines.push('');
  lines.push('- `SOMIX.Grouping` — containers (folders, modules, packages, classes)');
  lines.push('- `SOMIX.Code` — executable code units (functions, methods)');
  lines.push('- `SOMIX.Data` — data definitions (variables, constants, fields)');
  lines.push('');
  lines.push('Relationship types:');
  lines.push('');
  lines.push('- `SOMIX.ParentChild` — containment (a Grouping contains Code/Data/Groupings)');
  lines.push('- `SOMIX.Call` — one Code element calls another');
  lines.push('- `SOMIX.Access` — Code reads or writes Data');
  lines.push('');
  lines.push('A SOMIX model is the complete graph for an analyzed codebase.');
  lines.push('');
  lines.push('## What is a diagram?');
  lines.push('');
  lines.push('A diagram is a focused subset of the SOMIX model — a hand-curated view that highlights');
  lines.push('one specific aspect of the architecture. Elements with the `AwN` flag have ALL their');
  lines.push('calls and accesses visible in the diagram. Elements without `AwN` appear as context only');
  lines.push('— they may have additional relationships in the code that are not shown in the diagram.');
  lines.push('');
  lines.push('## File types');
  lines.push('');
  lines.push('Each diagram has the same base name and one to three files:');
  lines.push('');
  lines.push('| Suffix | Always present | Content |');
  lines.push('|--------|----------------|---------|');
  lines.push('| `.m2m`  | yes | Generation info used by Moose2Model2 to recreate the diagram (positions, comments, settings). JSON-formatted internally. |');
  lines.push('| `.json` | yes | AI-readable export of the diagram content: nodes, calls, accesses, comments. **Read this file to understand the diagram.** |');
  lines.push('| `.md`   | no  | Explanation of the diagram. May be written by a human or by an AI. When present, read it for intent and observations that are not derivable from the structural data alone. |');
  lines.push('');
  lines.push('## Editing a .m2m file directly');
  lines.push('');
  lines.push('A `.m2m` file is JSON. A unique key has the form `<TechnicalType>..<UniqueName>` and must');
  lines.push('exactly match a key that exists in the model (for example `JSFunction../src/Drawing.js/PinAll`).');
  lines.push('');
  lines.push('An element is shown in the regenerated diagram **only** if at least one of these holds:');
  lines.push('');
  lines.push('- its unique key is listed in `addedWithNeighbors`, or');
  lines.push('- it is a direct neighbor of an element in `addedWithNeighbors` — it calls or is called by');
  lines.push('  it, accesses or is accessed by it, or is a child of it. (Parents are *not* pulled in');
  lines.push('  automatically.)');
  lines.push('');
  lines.push('`addedWithNeighbors` is the internal field behind the user-facing `AwN` flag described above.');
  lines.push('');
  lines.push('The remaining arrays only attach data to, or remove, elements; on regeneration the tool');
  lines.push('silently ignores any entry whose element is not already in the diagram:');
  lines.push('');
  ...
  lines.push('');
  lines.push('So to keep an element that is **not** a neighbor of any seeded element, add its unique key to');
  lines.push('`addedWithNeighbors`. Listing it only in `positions`, `pinned`, or `comments` has no effect.');
  lines.push('');
  lines.push('## Available diagrams');
  lines.push('');
  if (entries.length === 0) {
    lines.push('_No diagrams exported._');
  } else {
    for (const e of entries) {
      const parts = [`\`${e.name}.m2m\``, `\`${e.name}.json\``];
      if (e.hasMd) parts.push(`\`${e.name}.md\``);
